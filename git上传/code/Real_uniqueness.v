...
Notation "x - y" := (x + (-y)) : R1_scope.
Notation "a ⁻" := (∩(\{ λ u, u ∈ (R ~ [0]) /\ u · a = 1 \})) : R1_scope.
Notation "m / n" := (m · (n⁻)) : R1_scope.
Notation "x < y" := (@lt R1 x y) : R1_scope.

Open Scope R2_scope.

Notation "0 '" := (@zeroR R2) : R2_scope.
Notation "1 '" := (@oneR R2) : R2_scope.
...
Local Hint Resolve PlusR zero_in_R MultR Plus_close Mult_close
...
Local Hint Resolve Plus_close zero_in_R Mult_close one_in_R one_in_R_Co
...
Theorem UniqueT1 : (∀ f, Function f -> dom(f) = R -> ran(f) ⊂ R'
  -> (∀ x y, x ∈ R -> y ∈ R -> f[(x + y)%r1] = (f[x] + f[y])%r2
    /\ f[(x · y)%r1] = (f[x] · f[y])%r2)
  -> f[0] = 0' /\ (ran(f) <> [0'] -> f[1] = 1')).
Proof.
  ...
  assert (f[(0 + 0)%r1] = f[0]). { rewrite Plus_P1; auto with real. }
  ...
  assert (f[(1 · 1)%r1] = f[1]). { rewrite Mult_P1; auto with real. }
...
Proof.
  ...
  apply (H3 1 (-(1))%r1) in H5 as []; auto with real.
...
Proof.
  ...
  as [H12 _]. pose proof H5. apply (H3 m (x / m)%r1) in H13 as [];
  auto with real. assert (f[(x / m)%r1] ∈ R').
...
Lemma UniqueT2_Lemma5 : ∀ n, n ∈ Z -> (0 < n)%r1 -> n ∈ N.
Proof.
...
Proof.
  ...
  { intros. apply MKT4 in H8 as [].
    ...
    assert (f[(-m)%r1] ∈ N'). { apply UniqueT2_Lemma1; auto. }
  ...
  { split. apply MKT126a; auto. split; unfold Relation; intros.
    ...
    - apply AxiomII' in H9 as [_],H10 as [_].
      ...
      { pose proof H0. apply UniqueT2_Lemma2 in H14; auto.
        ...
        apply (H3 (-(1))%r1 z) in H15 as []; auto with real.
  ...
  { destruct H9. apply AxiomI; split; intros.
    ...
    assert (f[(((f|(Z))⁻¹)[(-z)%r2] · (-(1)))%r1] = -z · -1').
    { rewrite H10 in H15. pose proof H15.
      apply Z_Subset_R,(H3 _ (-(1))%r1) in H16 as [_];
    ...
    apply (@ Property_ran (((f|(Z))⁻¹)[(-z)%r2] · -(1))%r1),
  ...
  { apply UniqueT2_Lemma5. apply Int_P1a; auto.
    ...
    apply (OrderPM_Co1 _ _ (-x)%r1) in H17; auto with real.
    rewrite Minus_P1 in H17; auto with real. }
  assert (f[(y - x)%r1] ∈ N'). { apply UniqueT2_Lemma1; auto. }
  pose proof H14. apply Z_Subset_R,(H3 y (-x)%r1) in H19
...
Theorem UniqueT3 : (∀ f, Function f -> dom(f) = R -> ran(f) ⊂ R'
  -> (∀ x y, x ∈ R -> y ∈ R -> f[(x + y)%r1] = (f[x] + f[y])%r2
    /\ f[(x · y)%r1] = (f[x] · f[y])%r2) -> ran(f) <> [0']
  -> (∀ m n, m ∈ Z -> n ∈ (Z ~ [0]) -> f[(m/n)%r1] = (f[m] / f[n])%r2)
    /\ Function1_1 (f|(Q)) /\ dom(f|(Q)) = Q /\ ran(f|(Q)) = Q'
    /\ (∀ x y, x ∈ Q -> y ∈ Q -> (x ≤ y)%r1 -> (f[x] ≤ f[y])%r2)).
Proof.
  assert (H:True) by auto.
  intros. pose proof H0. apply UniqueT1 in H5 as []; auto.
  assert (∀ m, m ∈ R -> f[m] ∈ R').
  ...
  { split. apply MKT126a; auto. split; unfold Relation; intros.
    ...
    - apply AxiomII' in H10 as [_],H11 as [_].
      ...
      { pose proof H0. apply UniqueT2_Lemma2 in H15; auto.
        ...
        apply (H3 (-(1))%r1 z) in H16 as []; auto with real.
  ...
  assert (x < y)%r1. split; auto.
  apply (OrderPM_Co1 _ _ (-x)%r1) in H25; auto with real.
  rewrite Minus_P1 in H25; auto with real.
  assert ((y - x)%r1 ∈ Q).
  { apply Rat_P1a; auto. apply Rat_P3; auto. }
  assert (0' < (f[(y - x)%r1])). { apply UniqueT3_Lemma2; auto. }
  pose proof H21. apply Q_Subset_R in H28.
  apply (H3 y (-x)%r1) in H28 as [H28 _]; auto with real.
...
Lemma UniqueT4_Lemma1 : ∀ r, r ∈ R' -> Sup1' \{ λ u, u ∈ Q' /\ u ≤ r \} r.
Proof.
...
Lemma UniqueT4_Lemma2 : (∀ f, Function f -> dom(f) = R -> ran(f) ⊂ R'
  -> (∀ x y, x ∈ R -> y ∈ R -> f[(x + y)%r1] = (f[x] + f[y])%r2
    /\ f[(x · y)%r1] = (f[x] · f[y])%r2) -> ran(f) <> [0']
  -> (∀ x y, x ∈ R -> y ∈ R -> (x < y)%r1 <-> (f[x] < f[y])%r2)).
Proof.
  assert (H:True) by auto. intros.
  assert (ReqR: (@Real_Axioms.R R1) = R) by auto.
  assert (ReqR': (@Real_Axioms.R R2) = R') by auto.
  assert (∀ r, r ∈ R -> (0 < r)%r1 -> 0' < f[r]).
  ...
  assert (∀ a b, a ∈ R -> b ∈ R -> (a < b)%r1 -> f[a] < f[b]).
  { intros. apply (OrderPM_Co1 _ _ (-a)%r1) in H11; auto with real.
    rewrite Minus_P1 in H11; auto. apply H7 in H11; auto with real.
    pose proof H10. apply (H3 b (-a)%r1) in H12 as []; auto with real.
...
Proof.
  ...
  assert (∀ x y, x ∈ R -> y ∈ R -> (x ≤ y)%r1 -> (f[x] ≤ f[y])%r2).
...
Proof.
 ...
 assert (NeqN': (@Real_Axioms.N R2) = N') by auto.
  ...
  assert (∀ m n, m ∈ N -> n ∈ N -> (m < n)%r1 -> f[m] < f[n]).
...
Lemma UniqueT5_Lemma2 : ∀ f, Function1_1 f -> dom(f) = N -> ran(f) = N'
  -> (∀ m n, m ∈ N -> n ∈ N -> (m < n)%r1 <-> f[m] < f[n])
  -> (∀ m n, m ∈ N -> n ∈ N -> f[(m + n)%r1] = f[m] + f[n]
    /\ f[(m · n)%r1] = f[m] · f[n] /\ ((m < n)%r1
      -> f[(n - m)%r1] = f[n] - f[m])) /\ f[1] = 1'.
Proof.
  intros f [] H1 H2 H3.
  assert (ReqR: (@Real_Axioms.R R1) = R) by auto.
  assert (ReqR': (@Real_Axioms.R R2) = R') by auto.
  ...
  assert (∀ m, m ∈ N -> f[(m + 1)%r1] = f[m] + f[1]).
  { intros. assert (f[m] < f[(m + 1)%r1]).
    ...
    assert (f[(m + 1)%r1] ∈ R'). auto with real.
    ...
    { destruct H14. apply (Order_Co2' _ (f[(m + 1)%r1]));
      auto with real. }
  ...
  assert (∀ m, m ∈ N -> (1 < m)%r1 -> f[(m - 1)%r1] = f[m] - f[1]).
  { intros. assert (f[(m - 1)%r1] < f[m]).
    { apply H3; auto. apply Nat_P2; destruct H9; auto.
      pose proof OrderPM_Co9. apply (OrderPM_Co1 _ _ (-(1))%r1)
    ...
    rewrite H14 in H13. assert ((m - 1)%r1 ∈ N).
  ...
  set (E := \{ λ u, u ∈ N /\ (∀ m, m ∈ N -> f[(m + u)%r1]
    = f[m] + f[u] /\ ((u < m)%r1 -> f[(m - u)%r1] = f[m] - f[u])) \}).
  assert (E ⊂ N /\ 1 ∈ E) as [].
  ...
  { apply MathInd; auto. intros. apply AxiomII in H11 as [_[]].
    apply AxiomII; repeat split; eauto with real.
    - assert (m + 1 ∈ N)%r1. auto with real. apply H12 in H14
    ...
    - intros. rewrite PlusMult_Co3,Mult_P4,Mult_P5,Mult_P4,
      <-PlusMult_Co3,Mult_P4,Mult_P1,(Plus_P4 (-x)%r1),Plus_P3;
      auto with real. assert (1 < m)%r1.
      { apply (Order_Co2 _ (x + 1)%r1); auto with real. left.
      ...
      apply (OrderPM_Co1 _ _ (-(1))%r1) in H14; auto with real.
      ...
      (Plus_P4' (-f[x])),Plus_P3'; auto with real'. }
  assert (∀ m n, m ∈ N -> n ∈ N -> f [(m + n)%r1] = f [m] + f [n]
    /\ ((m < n)%r1 -> f [(n - m)%r1] = f [n] - f [m])).
...
Proof.
  ...
  { apply AxiomI; split; intros.
    ...
    apply AxiomII in H7 as []. exists (-f[(-z)%r1]).
  ...
  { intros. pose proof H11. apply N_Subset_Z in H11. rewrite ZeqZ in *;
    ...
    elim zero_not_in_N; auto. }
  assert (∀ m, m ∈ \{ λ a, (-a)%r1 ∈ N \} -> g[m] = -f[(-m)%r1]).
  { intros. assert (m ∈ Z).
  ...
  assert (∀ m n, m ∈ N -> n ∈ \{ λ a, (-a)%r1 ∈ N \}
    -> g[(m + n)%r1] = g[m] + g[n] /\ g[(m · n)%r1] = g[m] · g[n]).
  { intros. assert (n ∈ Z).
    { apply MKT4; right; apply MKT4; auto. }
    assert ((m · n)%r1 ∈ \{ λ a, (-a)%r1 ∈ N \}) as Ha.
    ...
    rewrite ReqR, ReqR' in *.
    rewrite <-H20,PlusMult_Co3,Mult_P3,(Mult_P4 m (-(1))%r1),
  ...
  assert (∀ m n, m ∈ \{ λ a, (-a)%r1 ∈ N \}
    -> n ∈ \{ λ a, (-a)%r1 ∈ N \} -> g[(m + n)%r1] = g[m] + g[n]
    /\ g[(m · n)%r1] = g[m] · g[n]).
  { intros. assert (m ∈ Z /\ n ∈ Z) as [].
    { split; apply MKT4; right; apply MKT4; auto. }
    assert ((m + n)%r1 ∈ \{ λ a, (-a)%r1 ∈ N \}).
    ...
    rewrite ReqR, ReqR' in *.
    rewrite H20,H22,PlusMult_Co3',(PlusMult_Co3' f[(-n)%r1]),
  ...
  rewrite H2 in H19. assert (0' < f[(-m)%r1]).
  { destruct one_is_min_in_N' as [_[]].
    apply (Order_Co2' _ 1'); auto with real'. }
  apply (OrderPM_Co1' _ _ (-f[(-m)%r1])) in H20; auto with real'.
  ...
  { pose proof H17. apply MKT4 in H17 as []; auto.
    ...
    { apply (Order_Co2 _ (-m)%r1); destruct H21; auto with real. }
...
Lemma UniqueT5_Lemma4 : ∃ f, Function f /\ dom(f) = Q
  /\ ran(f) ⊂ Q' /\ f[0] = 0' /\ f[1] = 1'
  /\ (∀ a b, a ∈ Q -> b ∈ Q -> f[(a + b)%r1] = f[a] + f[b]
    /\ f[(a · b)%r1] = f[a] · f[b])
  /\ (∀ a b, a ∈ Q -> b ∈ Q -> (a < b)%r1 -> f[a] < f[b]).
Proof.
  ...
  { intros. assert ((b · d) · (a / (1 ·b)) = (b · d) · (c / (1 · d)))%r1.
    ...
    assert (f[(d · a)%r1] = f[d] · f[a] /\ f[(c · b)%r1] = f[c] · f[b])
  ...
  assert (∀ a b, a ∈ Z -> b ∈ (Z ~ [0]) -> g[(a / b)%r1]
    = f[a] / f[b]).
  { intros. assert ((a / b)%r1 ∈ Q).
  ...
  assert (ReqR: (@Real_Axioms.R R1) = R) by auto.
  assert (ReqR': (@Real_Axioms.R R2) = R') by auto.
  assert (∀ a b, a ∈ Q -> b ∈ Q -> g[(a + b)%r1] = g[a] + g[b]
    /\ g[(a · b)%r1] = g[a] · g[b]).
  ...
  { intros. apply Existence_of_irRational_Number_Lemma6 in H27
    ...
    rewrite NeqN, NeqN' in *.
    left. split; auto. }
  assert (∀ a b, a ∈ Q -> b ∈ Q -> (a < b)%r1 -> g[a] < g[b]).
  { intros. apply (OrderPM_Co1 _ _ (-a)%r1) in H30; auto with real.
...
Lemma UniqueT5_Lemma7 : ∃ f, Function f
  /\ dom(f) = \{ λ u, u ∈ R /\ (0 < u)%r1 \} /\ ran(f) ⊂ R'
  /\ (∀ m n, m ∈ dom(f) -> n ∈ dom(f) -> f[(m + n)%r1] = f[m] + f[n])
  /\ (∀ m n, m ∈ dom(f) -> n ∈ dom(f) -> f[(m · n)%r1] = f[m] · f[n])
...
Proof.
  ...
  set (A r := \{ λ u, ∃ x, x ∈ \{ λ v, v ∈ Q /\ (0 < v)%r1
    /\ (v < r)%r1 \} /\ u = f[x] \}).
  ...
  set (g := \{\ λ u v, u ∈ R /\ (0 < u)%r1 /\ Sup1' (A u) v \}\).
  assert (Function g).
  { split; unfold Relation; intros.
    apply AxiomII in H6 as [_[x[y[]]]]; eauto.
  ...
  { apply AxiomI; split; intros. apply AxiomII in H7 as [_[]].
    ...
    { apply SupT'; auto. apply NEexE. apply AxiomII in H7 as [_[]].
      ...
      apply Property_Value,Property_ran in H11; auto.
      exists f[(m + 1)%r1]. repeat split; auto with real'. intros.
  ...
  assert (∀ a b, a ∈ Q -> b ∈ R -> (0 < a)%r1 -> (0 < b)%r1
    -> (a < b)%r1 -> f[a] ≤ g[b]) as Hb.
  ...
  { intros. assert ((m + n)%r1 ∈ dom(g)).
    { rewrite H7. apply AxiomII; split; [exists R|split].
      ...
      replace 0 with (0 + 0)%r1. apply OrderPM_Co4; auto with real.
      destruct H15; auto. rewrite Plus_P1; auto with real. }
    assert (g[(m + n)%r1] ∈ R'); auto.
    assert ((g[m] + g[n]) ∈ R'). auto with real'.
    pose proof H17. apply (Order_Co1' g[(m + n)%r1]) in H18
    as [H18|[]]; auto.
    - pose proof H18. apply UniqueT5_Lemma5b in H19
      ...
      assert (f[(x1 + x2)%r1] = f[x1] + f[x2]). { apply H4; auto. }
  ...
  { intros. assert ((m · n)%r1 ∈ dom(g)).
    ...
    assert (g[(m · n)%r1] ∈ R'); auto.
    assert ((g[m] · g[n]) ∈ R'). auto with real'.
    pose proof H18. apply (Order_Co1' g[(m · n)%r1]) in H19
    as [H19|[]]; auto.
    - apply UniqueT5_Lemma6b in H19
      ...
      { apply (OrderPM_Co7a _ _ x2) in H33; auto with real.
        ...
        apply (Order_Co2 _ (m · x2)%r1); auto with real.
        destruct H33; auto. }
  ...
  { intros. assert ((n - m)%r1 ∈ dom(g)).
    { rewrite H7 in *. apply AxiomII in H15 as [_[]],H16 as [_[]].
      ...
      apply (OrderPM_Co1 _ _ (-m)%r1) in H17; auto with real.
      rewrite Minus_P1 in H17; auto. }
    assert (g[(n - m + m)%r1] = g[(n - m)%r1] + g[m]).
    { apply H13; auto. }
    rewrite <-Plus_P3,(Plus_P4 (-m)%r1),Minus_P1,Plus_P1 in H19;
    auto with real. rewrite H19,<-Plus_P3',Minus_P1',Plus_P1';
    auto with real'. }
  assert (g[1] = 1').
  { assert (1 ∈ dom(g)).
    { rewrite H7; apply AxiomII; split; eauto with real. }
    assert (g[(1 · 1)%r1] = g[1] · g[1]). { apply H14; auto. }
    assert (g[1] ∈ (R' ~ [0'])).
    { apply MKT4'; split; auto. apply AxiomII; split; eauto.
...
Proof.
  ...
  set (g := \{\ λ u v, u ∈ R /\ (((0 < u)%r1 /\ v = f[u])
    \/ (u = 0 /\ v = 0') \/ ((u < 0)%r1 /\ v = -f[(-u)%r1])) \}\).
  assert (∀ m, m ∈ R -> (0 < m)%r1 -> f[m] ∈ R') as Hf.
  { intros. apply H1,(@ Property_ran m),Property_Value; auto.
    rewrite H0. apply AxiomII; split; eauto. }
  assert (∀ m, m ∈ R -> (m < 0)%r1 -> (0 < -m)%r1) as Hb.
  { intros. apply (OrderPM_Co1 _ _ (-m)%r1) in H7; auto with real.
  ...
  { apply AxiomI; split; intros.
    ...
    - exists (-f[(-z)%r1]). apply AxiomII'; split; auto.
  ...
  assert (∀ m, m ∈ R -> (m < 0)%r1 -> g[m] = -f[(-m)%r1]).
  ...
  { intros. rewrite (H10 m),(H11 n); auto.
    ...
    - rewrite H11; auto with real. split. rewrite PlusMult_Co3,
      ...
      auto with real real'.
      apply (OrderPM_Co1 _ _ (-n)%r1) in H20; auto with real.
    ...
    - rewrite H20. rewrite Plus_P4 in H20; auto.
      ...
      rewrite Mult_P4; auto. apply OrderPM_Co6; auto. }
  assert (∀ m n, m ∈ R -> n ∈ R -> (m < 0)%r1 -> (n < 0)%r1
    -> g[(m + n)%r1] = g[m] + g[n] /\ g[(m · n)%r1] = g[m] · g[n]).
  { intros. rewrite (H11 m),(H11 n); auto.
    ...
    { rewrite ReqR, ReqR' in *.
      rewrite PlusMult_Co3,(PlusMult_Co3 n),Mult_P3,
      (Mult_P4 (-(1))%r1),<-(Mult_P3 m),PlusMult_Co5,Mult_P1,
      Mult_P1; auto with real. } rewrite ReqR, ReqR' in *.
    rewrite H22. rewrite H10,H3,PlusMult_Co3',
    (PlusMult_Co3' f[(-n)%r1]),Mult_P3',(Mult_P4' (-1')),
    <-(Mult_P3' f[(-m)%r1]),PlusMult_Co5',Mult_P1',Mult_P1';
    auto with real real'.  }
  assert (∀ m n, m ∈ dom(g) -> n ∈ dom(g)
    -> g[(m + n)%r1] = g[m] + g[n]).
  { intros.
...
Definition reals_isomorphism := ∃ f, Function1_1 f /\ dom(f) = R /\ ran(f) = R'
  /\ (∀ x y, x ∈ R -> y ∈ R -> f[(x + y)%r1] = (f[x] + f[y])%r2
    /\ f[(x · y)%r1] = (f[x] · f[y])%r2 /\ ((x ≤ y)%r1 <-> (f[x] ≤ f[y])%r2)).
